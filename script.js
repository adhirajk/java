...
function populateNavbar() {
  ...
  categories.forEach((category, idx) => {
    ...
    button.onclick = () => {
      makeInActive(".navbar button");
      button.classList.add("active");
      populateSidebar(category);
    };
    navbar.appendChild(button);
  });
  populateSidebar("basic");
}

function makeInActive(selector) {
...
function populateSidebar(category) {
  ...
  questions.forEach((item) => {
    ...
    button.onclick = () => {
      makeInActive(".sidebar button");
      button.classList.add("active");
      displayCode(item.answer);
    };
    sidebar.appendChild(button);
  });
}

function displayCode(answer) {
  document.getElementById("code").textContent = answer;
}
